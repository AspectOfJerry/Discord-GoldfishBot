    ...
    callback: (message, Discord, client, ...args) => {
        ...
        if(args[0] == '?') {
            const help_command = new Discord.MessageEmbed()
                ...
                .setFooter({text: "./commands/kick.js; Lines: 172; File size: ~8.11 KB"})
        ...
        if(!args[0]) {
            const error_missing_arguments = new Discord.MessageEmbed()
                .setColor('#ff2020')
                .setThumbnail(`${message.author.displayAvatarURL({dynamic: true, size: 16})}`)
                .setDescription(`**Error:** Excpected **${EXCPECTED_ARGUMENTS}** arguments but only provided **0**.`)
        ...
        if(!target) {
            const reference_error_target = new Discord.MessageEmbed()
                .setColor('ff2020')
                .setThumbnail(`${message.author.displayAvatarURL({dynamic: true, size: 16})}`)
                .setDescription("**Error:** Please provide a valid member to kick.")
                .setFooter({text: " Use " + "`" + `%${COMMAND_NAME} ?` + "`" + " for help."})
        ...
        if(memberTarget == message.member) {
            const error_cannot_use_on_self = new Discord.MessageEmbed()
                .setColor('ff2020')
                .setThumbnail(`${message.author.displayAvatarURL({dynamic: true, size: 16})}`)
                .setDescription('**Error:** You cannot use this command on yourself. Kick someone else!')
